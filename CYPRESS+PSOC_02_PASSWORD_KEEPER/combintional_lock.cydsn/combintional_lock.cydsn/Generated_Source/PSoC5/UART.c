...
uint8 UART_initVar = 0u;
    ...
    #if (CY_PSOC4)
        static void UART_InitEpDma(void);

        /* DMA chanels assigend for endpoints. */
        const uint8 UART_DmaChan[UART_MAX_EP] =
        {
            0u,
            0u,
            0u,
    ...
    uint8  UART_DmaEpBurstCnt   [UART_MAX_EP];
    
    /* Number of bytes to transfer in last DMA burst. */
    uint8  UART_DmaEpLastBurstEl[UART_MAX_EP];

    /* Storage for arrays above. */
    uint8  UART_DmaEpBurstCntBackup  [UART_MAX_EP];
    uint32 UART_DmaEpBufferAddrBackup[UART_MAX_EP];
     
    /* DMA trigger mux output for usb.dma_req[0-7]. */
    const uint8 UART_DmaReqOut[UART_MAX_EP] =
    {
        0u,
        UART_ep1_dma__TR_OUTPUT,
        UART_ep2_dma__TR_OUTPUT,
        UART_ep3_dma__TR_OUTPUT,
        0u,
        0u,
        0u,
        0u,
    ...
    const uint8 UART_DmaBurstEndOut[UART_MAX_EP] =
    {
        0u,
        UART_BURSTEND_0_TR_OUTPUT,
        UART_BURSTEND_1_TR_OUTPUT,
        UART_BURSTEND_2_TR_OUTPUT,
        UART_BURSTEND_3_TR_OUTPUT,
        UART_BURSTEND_4_TR_OUTPUT,
        UART_BURSTEND_5_TR_OUTPUT,
        UART_BURSTEND_6_TR_OUTPUT,
        UART_BURSTEND_7_TR_OUTPUT
    };
...
*   following list.
*
*       *UART_3V_OPERATION* - Disable voltage regulator and pass-
*                                      through Vcc for pull-up
*
*       *UART_5V_OPERATION* - Enable voltage regulator and use
*                                      regulator for pull-up
*
*       *UART_DWR_POWER_OPERATION* - Enable or disable the voltage
...
* UART_Init() because the UART_Start() routine calls
...
void UART_Init(void) 
{
    ...
    * UART_InitComponent() if it is required.
    */

    /* Enable USBIO control on drive mode of D+ and D- pins. */
    UART_USBIO_CR1_REG &= ~ (uint32) UART_USBIO_CR1_IOMODE;

    /* Set number of LF CLK to detect UBS bus reset. */
    UART_BUS_RST_CNT_REG = UART_DEFUALT_BUS_RST_CNT;
    ...
    */
    UART_POWER_CTRL_REG = UART_DEFAULT_POWER_CTRL_VBUS;

    /* Enable PHY detector and single-ended and differential receivers. */
    UART_POWER_CTRL_REG |= UART_DEFAULT_POWER_CTRL_PHY;
    ...
    UART_INTR_SIE_MASK_REG = UART_DEFAULT_INTR_SIE_MASK;
    ...
    */
    ...
    CyDelayUs(UART_WAIT_REG_STABILITY_1US);
    ...
    CyIntSetPriority     (UART_INTR_HI_VECT_NUM,  UART_INTR_HI_PRIORITY);
    (void) CyIntSetVector(UART_INTR_HI_VECT_NUM, &UART_INTR_HI_ISR);

    /* Configure lo_int: set handler and priority. */
    CyIntSetPriority     (UART_INTR_LO_VECT_NUM,  UART_INTR_LO_PRIORITY);
    (void) CyIntSetVector(UART_INTR_LO_VECT_NUM, &UART_INTR_LO_ISR);

    /* Configure med_int: set handler and priority (routed through DSI). */
    CyIntSetPriority     (UART_INTR_MED_VECT_NUM,  UART_INTR_MED_PRIORITY);
    (void) CyIntSetVector(UART_INTR_MED_VECT_NUM, &UART_INTR_MED_ISR);
    ...
    #endif /* (UART_EP_MANAGEMENT_DMA && UART_ARB_ISR_ACTIVE) */
#endif /* (CY_PSOC4) */

    /* Common: Configure GPIO interrupt for wakeup. */
#if (UART_DP_ISR_ACTIVE)
    CyIntSetPriority     (UART_DP_INTC_VECT_NUM,  UART_DP_INTC_PRIORITY);
    (void) CyIntSetVector(UART_DP_INTC_VECT_NUM, &UART_DP_ISR);
#endif /* (UART_DP_ISR_ACTIVE) */

#if (UART_EP_MANAGEMENT_DMA && CY_PSOC4)
    /* Initialize DMA channels. */
    UART_InitEpDma();
#endif /* (UART_EP_MANAGEMENT_DMA && CY_PSOC4) */
}
...
* \globalvars
*   \ref UART_device
*   \ref UART_transferState
*   \ref UART_configuration
*   \ref UART_deviceStatus
*
*   \ref UART_deviceAddress - Contains the current device address. This
...
*      SET_ADDRESS request.
*
*   \ref UART_lastPacketSize - Initialized to 0;
*
* \reentrant
*  No.
*
...
#if (CY_PSOC4)
    CyIntEnable(UART_INTR_HI_VECT_NUM);
    CyIntEnable(UART_INTR_MED_VECT_NUM);
    CyIntEnable(UART_INTR_LO_VECT_NUM);
#else
    CyIntEnable(UART_BUS_RESET_VECT_NUM);
    CyIntEnable(UART_EP_0_VECT_NUM);

    #if (UART_SOF_ISR_ACTIVE)
        CyIntEnable(UART_SOF_VECT_NUM);
    #endif /* (UART_SOF_ISR_ACTIVE) */
    ...
    UART_ARB_CFG_REG = UART_DEFAULT_ARB_CFG;
        ...
        CyDmaEnable();
            ...
            /* Change DMA priority to be highest. */
             CyIntSetPriority(CYDMA_INTR_NUMBER, UART_DMA_AUTO_INTR_PRIO);
        #endif /* (UART_EP_MANAGEMENT_DMA_AUTO) */
    #endif /* (CY_PSOC4) */
...
*       in the Device Number field.
*   UART_transferState - This variable is used by the communication
*       functions to handle the current transfer state. Initialized to
*       TRANS_STATE_IDLE in this API.
*   UART_configuration - Contains the current configuration number
*       set by the Host using a SET_CONFIGURATION request.
*       Initialized to zero in this API.
*   UART_deviceAddress - Contains the current device address. This
*       variable is initialized to zero in this API. The Host starts to communicate
...
*   \ref UART_deviceStatus
*
*   \ref UART_configurationChanged
*
*   UART_intiVar -  This variable is set to zero
*
*******************************************************************************/
void UART_Stop(void) 
{
...
#if (CY_PSOC4)
    /* Disable USBFS block. */
    UART_POWER_CTRL_REG &= (uint32) ~UART_POWER_CTRL_ENABLE;
#else
...
#if (CY_PSOC4)
    CyIntDisable(UART_INTR_HI_VECT_NUM);
    CyIntDisable(UART_INTR_LO_VECT_NUM);
    CyIntDisable(UART_INTR_MED_VECT_NUM);
#else
    ...
    #if (UART_SOF_ISR_ACTIVE)
        CyIntDisable(UART_SOF_VECT_NUM);
    #endif /* (UART_SOF_ISR_ACTIVE) */
...
{
    uint8 cr1Reg = UART_CR1_REG;

    /* Clear bus activity. */
    UART_CR1_REG = (cr1Reg & (uint8) ~UART_CR1_BUS_ACTIVITY);

    /* Get bus activity. */
    return ((0u != (cr1Reg & UART_CR1_BUS_ACTIVITY)) ? (1u) : (0u));
}
...
* \globalvars
*
*  \ref UART_configurationChanged - This variable is set to 1 after
...
uint16 UART_GetEPCount(uint8 epNumber) 
{
    ...
    if ((epNumber > UART_EP0) && (epNumber < UART_MAX_EP))
    {
        ...
        cntr  = ((uint16) UART_SIE_EP_BASE.sieEp[epNumber].epCnt0) & UART_EPX_CNT0_MASK;
        cntr  = ((uint16) (cntr << 8u)) | ((uint16) UART_SIE_EP_BASE.sieEp[epNumber].epCnt1);
        cntr -= UART_EPX_CNTX_CRC_COUNT;
    }
    ...
    *******************************************************************************/
    ...
    {
    #if (UART_DMA1_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[UART_ep1_dma_CHANNEL] = UART_ep1_dma_CHANNEL_CFG;
    #endif /* (UART_DMA1_ACTIVE) */

    #if (UART_DMA2_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[UART_ep2_dma_CHANNEL] = UART_ep2_dma_CHANNEL_CFG;
    #endif /* (UART_DMA2_ACTIVE) */

    #if (UART_DMA3_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[UART_ep3_dma_CHANNEL] = UART_ep3_dma_CHANNEL_CFG;
    #endif /* (UART_DMA3_ACTIVE) */

    #if (UART_DMA4_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[UART_ep4_dma_CHANNEL] = UART_ep4_dma_CHANNEL_CFG;
    #endif /* (UART_DMA4_ACTIVE) */

    #if (UART_DMA5_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[UART_ep5_dma_CHANNEL] = UART_ep5_dma_CHANNEL_CFG;
    #endif /* (UART_DMA5_ACTIVE) */

    #if (UART_DMA6_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[UART_ep6_dma_CHANNEL] = UART_ep6_dma_CHANNEL_CFG;
    #endif /* (UART_DMA6_ACTIVE) */

    #if (UART_DMA7_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[UART_ep7_dma_CHANNEL] = UART_ep7_dma_CHANNEL_CFG;
    #endif /* (UART_DMA7_ACTIVE) */

    #if (UART_DMA8_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[UART_ep8_dma_CHANNEL] = UART_ep8_dma_CHANNEL_CFG;
    #endif /* (UART_DMA8_ACTIVE) */

    #if (UART_EP_MANAGEMENT_DMA_AUTO)
        /* Initialize DMA channel callbacks. */
        #if (UART_DMA1_ACTIVE)
            (void) UART_ep1_dma_SetInterruptCallback(&UART_EP1_DMA_DONE_ISR);
        #endif /* (UART_DMA1_ACTIVE) */

        #if (UART_DMA2_ACTIVE)
            (void) UART_ep2_dma_SetInterruptCallback(&UART_EP2_DMA_DONE_ISR);
        #endif /* (UART_DMA2_ACTIVE) */

        #if (UART_DMA3_ACTIVE)
            (void) UART_ep3_dma_SetInterruptCallback(&UART_EP3_DMA_DONE_ISR);
        #endif /* (UART_DMA3_ACTIVE) */

        #if (UART_DMA4_ACTIVE)
            (void) UART_ep4_dma_SetInterruptCallback(&UART_EP4_DMA_DONE_ISR);
        #endif /* (UART_DMA4_ACTIVE) */

        #if (UART_DMA5_ACTIVE)
            (void) UART_ep5_dma_SetInterruptCallback(&UART_EP5_DMA_DONE_ISR);
        #endif /* (UART_DMA5_ACTIVE) */

        #if (UART_DMA6_ACTIVE)
            (void) UART_ep6_dma_SetInterruptCallback(&UART_EP6_DMA_DONE_ISR);
        #endif /* (UART_DMA6_ACTIVE) */

        #if (UART_DMA7_ACTIVE)
            (void) UART_ep7_dma_SetInterruptCallback(&UART_EP7_DMA_DONE_ISR);
        #endif /* (UART_DMA7_ACTIVE) */

        #if (UART_DMA8_ACTIVE)
            (void) UART_ep8_dma_SetInterruptCallback(&UART_EP8_DMA_DONE_ISR);
        #endif /* (UART_DMA8_ACTIVE) */
    #endif /* (UART_EP_MANAGEMENT_DMA_AUTO) */
    }
#else
    ...
    ***************************************************************************/
    ...
    #if (UART_DMA1_ACTIVE)
        case UART_EP1:
            UART_DmaChan[epNumber] = UART_ep1_DmaInitialize(UART_DMA_BYTES_PER_BURST,
                                                                                    UART_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (UART_DMA1_ACTIVE) */

    #if (UART_DMA2_ACTIVE)
        case UART_EP2:
            UART_DmaChan[epNumber] = UART_ep2_DmaInitialize(UART_DMA_BYTES_PER_BURST,
                                                                                    UART_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (UART_DMA2_ACTIVE) */
    ...
    #if (UART_DMA3_ACTIVE)
            case UART_EP3:
                UART_DmaChan[epNumber] = UART_ep3_DmaInitialize(UART_DMA_BYTES_PER_BURST,
                                                                                        UART_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (UART_DMA3_ACTIVE) */
    ...
    #if (UART_DMA4_ACTIVE)
            case UART_EP4:
                UART_DmaChan[epNumber] = UART_ep4_DmaInitialize(UART_DMA_BYTES_PER_BURST,
                                                                                        UART_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (UART_DMA4_ACTIVE) */
    ...
    #if (UART_DMA5_ACTIVE)
            case UART_EP5:
                UART_DmaChan[epNumber] = UART_ep5_DmaInitialize(UART_DMA_BYTES_PER_BURST,
                                                                                        UART_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (UART_DMA5_ACTIVE) */
    ...
    #if (UART_DMA6_ACTIVE)
        case UART_EP6:
            UART_DmaChan[epNumber] = UART_ep6_DmaInitialize(UART_DMA_BYTES_PER_BURST,
                                                                                    UART_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (UART_DMA6_ACTIVE) */
    ...
    #if (UART_DMA7_ACTIVE)
        case UART_EP7:
                UART_DmaChan[epNumber] = UART_ep7_DmaInitialize(UART_DMA_BYTES_PER_BURST,
                                                                                        UART_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (UART_DMA7_ACTIVE) */
    ...
    #if (UART_DMA8_ACTIVE)
        case UART_EP8:
                UART_DmaChan[epNumber] = UART_ep8_DmaInitialize(UART_DMA_BYTES_PER_BURST,
                                                                                        UART_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (UART_DMA8_ACTIVE) */
    ...
    *  to trigger DMA re-configuration when UART_LoadInEP() or 
    *  UART_ReadOutEP() functions are called the first time. 
    *  
    ...
    ***************************************************************************/
    void UART_Stop_DMA(uint8 epNumber) 
    {
        ...
        {
        #if (CY_PSOC4)
            if (0u != UART_DmaChan[i])
            {
                UART_CyDmaChDisable(UART_DmaChan[i]);
            }
        #else
                                                                        ...
                                                                        
{
    if ((epNumber > UART_EP0) && (epNumber < UART_MAX_EP))
    {
        ...
        UART_SIE_EP_BASE.sieEp[epNumber].epCnt0 = (uint8) HI8(length) | UART_EP[epNumber].epToggle;
        UART_SIE_EP_BASE.sieEp[epNumber].epCnt1 = (uint8) LO8(length);
        ...
        if (NULL != pData)
        {
            ...
            for (i = 0u; i < length; ++i)
            {
                UART_ARB_EP_BASE.arbEp[epNumber].rwDr = pData[i];
            }
        }
        ...
        /* Arm IN endpoint. */
        UART_SIE_EP_BASE.sieEp[epNumber].epCr0 = UART_EP[epNumber].epMode;

    #else

        #if (CY_PSOC3 || CY_PSOC5LP)
            /* Initialize DMA if it was not initialized. */
            if (DMA_INVALID_TD == UART_DmaTd[epNumber])
            {
            ...
            {
            #if (CY_PSOC4)
                uint32 channelNum = (uint32) UART_DmaChan[epNumber];

                /* Disable DMA channel: start configuration. */
                UART_CyDmaChDisable(channelNum);

                /* Configure source and destination. */
                UART_CyDmaSetSrcAddress(channelNum, UART_DMA_DESCR0, (void*) pData);
                UART_CyDmaSetDstAddress(channelNum, UART_DMA_DESCR0, (void*) &UART_ARB_EP_BASE.arbEp[epNumber].rwDr);

                /* Configure DMA descriptor. */
                --length;
                UART_CyDmaSetConfiguration(channelNum, UART_DMA_DESCR0, UART_DMA_COMMON_CFG | length |
                ...
                (void) CyDmaTdSetConfiguration(UART_DmaTd[epNumber], length, CY_DMA_DISABLE_TD, TD_TERMIN_EN | TD_INC_SRC_ADR);
                (void) CyDmaTdSetAddress(UART_DmaTd[epNumber], LO16((uint32) pData), LO16((uint32) &UART_ARB_EP_BASE.arbEp[epNumber].rwDr));
                ...
                UART_ARB_EP_BASE.arbEp[epNumber].epCfg |=  (uint8)  UART_ARB_EPX_CFG_DMA_REQ;
                UART_ARB_EP_BASE.arbEp[epNumber].epCfg &=  (uint8) ~UART_ARB_EPX_CFG_DMA_REQ;
            ...
            if (pData != NULL)
            {
                ...
                UART_DmaEpBufferAddrBackup[epNumber] = (uint32) pData;
                ...
                UART_CyDmaSetDstAddress(channelNum, UART_DMA_DESCR0, (void*) &UART_ARB_EP_BASE.arbEp[epNumber].rwDr);
                UART_CyDmaSetDstAddress(channelNum, UART_DMA_DESCR1, (void*) &UART_ARB_EP_BASE.arbEp[epNumber].rwDr);

                /* Configure DMA descriptor. */
                UART_CyDmaSetConfiguration(channelNum, UART_DMA_DESCR0, UART_DMA_COMMON_CFG  |
                                                        CYDMA_BYTE | CYDMA_ELEMENT_WORD | CYDMA_INC_SRC_ADDR | CYDMA_INVALIDATE | CYDMA_CHAIN);

                /* Configure DMA descriptor. */
                UART_CyDmaSetConfiguration(channelNum, UART_DMA_DESCR1, UART_DMA_COMMON_CFG  |
                                                        CYDMA_BYTE | CYDMA_ELEMENT_WORD | CYDMA_INC_SRC_ADDR | CYDMA_INVALIDATE | CYDMA_CHAIN);

                /* Enable interrupt from DMA channel. */
                UART_CyDmaSetInterruptMask(channelNum);
                ...
                (void) CyDmaChDisable(UART_DmaChan[epNumber]);
                    ...
                    (void) CyDmaTdSetAddress(UART_DmaTd[epNumber], LO16((uint32) pData),
                                                                               LO16((uint32) &UART_ARB_EP_BASE.arbEp[epNumber].rwDr));
                    ...
                    */
                    ...
                    (void) CyDmaTdSetAddress(UART_DmaNextTd[epNumber], LO16((uint32) &clearInDataRdyStatus),
                                                                                   LO16((uint32) &UART_ARB_EP_BASE.arbEp[epNumber].epCfg));
                #else
            ...
            else
            {
                ...
                if (length > 0u)
                {
                    ...
                    UART_DmaEpBurstCnt[epNumber] = (uint8) (length / UART_DMA_BYTES_PER_BURST);

                    /* Get number of elements in the last burst. */
                    UART_DmaEpLastBurstEl[epNumber] = (uint8) (length % UART_DMA_BYTES_PER_BURST);

                    /* Get total number of bursts. */
                    UART_DmaEpBurstCnt[epNumber] += (0u != UART_DmaEpLastBurstEl[epNumber]) ? 1u : 0u;

                    /* Adjust number of data elements transferred in last burst. */
                    UART_DmaEpLastBurstEl[epNumber] = (0u != UART_DmaEpLastBurstEl[epNumber]) ?
                                                                          (UART_DmaEpLastBurstEl[epNumber] - 1u) :
                                                                          (UART_DMA_BYTES_PER_BURST - 1u);

                    /* Get number of data elements to transfer for descriptor 0 and 1. */
                    lengthDescr0 = (1u == UART_DmaEpBurstCnt[epNumber]) ? UART_DmaEpLastBurstEl[epNumber] : (UART_DMA_BYTES_PER_BURST - 1u);
                    lengthDescr1 = (2u == UART_DmaEpBurstCnt[epNumber]) ? UART_DmaEpLastBurstEl[epNumber] : (UART_DMA_BYTES_PER_BURST - 1u);


                    /* Mark which descriptor is last one to execute. */
                    UART_DmaEpLastBurstEl[epNumber] |= (0u != (UART_DmaEpBurstCnt[epNumber] & 0x1u)) ?
                                                                            UART_DMA_DESCR0_MASK : UART_DMA_DESCR1_MASK;

                    /* Restore DMA settings for current transfer. */
                    UART_CyDmaChDisable(channelNum);

                    /* Restore destination address for input endpoint. */
                    UART_CyDmaSetSrcAddress(channelNum, UART_DMA_DESCR0, (void*) ((uint32) UART_DmaEpBufferAddrBackup[epNumber]));
                    UART_CyDmaSetSrcAddress(channelNum, UART_DMA_DESCR1, (void*) ((uint32) UART_DmaEpBufferAddrBackup[epNumber] +
                                                                                                                   UART_DMA_BYTES_PER_BURST));

                    /* Set number of elements to transfer. */
                    UART_CyDmaSetNumDataElements(channelNum, UART_DMA_DESCR0, lengthDescr0);
                    UART_CyDmaSetNumDataElements(channelNum, UART_DMA_DESCR1, lengthDescr1);
                    ...
                    if (UART_DmaEpBurstCnt[epNumber] > 1u)
                    {
                    ...
                    UART_DmaEpBurstCnt[epNumber] = UART_DMA_GET_BURST_CNT(UART_DmaEpBurstCnt[epNumber]);

                    /* Enable DMA channel: configuration complete. */
                    UART_CyDmaChEnable(channelNum);

                #elif (UART_EP_DMA_AUTO_OPT == 0u)
                    UART_inLength[epNumber]  = length;
                    ...
                    (void) CyDmaTdSetConfiguration(
                        ...
                        UART_DmaNextTd[epNumber], TD_TERMIN_EN | TD_INC_SRC_ADR );
                    (void) CyDmaTdSetAddress(UART_DmaTd[epNumber], LO16((uint32)  UART_inDataPointer[epNumber]),
                                                                               LO16((uint32) &UART_ARB_EP_BASE.arbEp[epNumber].rwDr));
                    /* Clear Any potential pending DMA requests before starting DMA channel to transfer data. */
                    (void) CyDmaClearPendingDrq(UART_DmaChan[epNumber]);
                    /* Enable DMA. */
                ...
                #if !defined (UART_MANUAL_IN_EP_ARM)
                    /* Set IN data ready to generate DMA request to load data into endpoint buffer. */
                    UART_ARB_EP_BASE.arbEp[epNumber].epCfg |= UART_ARB_EPX_CFG_IN_DATA_RDY;
...
*   transfer after calling the UART_ReadOutEP() API. For example,
*   by checking EPstate:
*
...
{
    if ((pData != NULL) && (epNumber > UART_EP0) && (epNumber < UART_MAX_EP))
    {
    #if (!UART_EP_MANAGEMENT_DMA_AUTO)
        /* Adjust requested length to available data. */
        length = (length > UART_GetEPCount(epNumber)) ? UART_GetEPCount(epNumber) : length;
    #endif /* (!UART_EP_MANAGEMENT_DMA_AUTO) */
    ...
    #if (UART_EP_MANAGEMENT_MANUAL)
        {
            ...
            for (i = 0u; i < length; ++i)
            {
                pData[i] = (uint8) UART_ARB_EP_BASE.arbEp[epNumber].rwDr;
            }
        }
        ...
        #if (CY_PSOC4)
        {
            ...
            /* Configure source and destination. */
            UART_CyDmaSetSrcAddress(channelNum, UART_DMA_DESCR0, (void*) &UART_ARB_EP_BASE.arbEp[epNumber].rwDr);
            UART_CyDmaSetDstAddress(channelNum, UART_DMA_DESCR0, (void*) pData);

            /* Configure DMA descriptor. */
            UART_CyDmaSetConfiguration(channelNum, UART_DMA_DESCR0, UART_DMA_COMMON_CFG | (uint16)(length - 1u) |
            ...
            /* Configure DMA to transfer data. */
            (void) CyDmaChDisable(UART_DmaChan[epNumber]);
            (void) CyDmaTdSetConfiguration(UART_DmaTd[epNumber], length, CY_DMA_DISABLE_TD, TD_TERMIN_EN | TD_INC_DST_ADR);
            (void) CyDmaTdSetAddress(UART_DmaTd[epNumber], LO16((uint32) &UART_ARB_EP_BASE.arbEp[epNumber].rwDr), LO16((uint32)pData));
        ...
        #if (CY_PSOC4)
        {
            ...
            UART_DmaEpLastBurstEl[epNumber] |= (UART_DmaEpBurstCnt[epNumber] > 2u) ? UART_DMA_DESCR_REVERT : 0u;
            ...
            UART_DmaEpBufferAddrBackup[epNumber] = (uint32) pData;
            UART_DmaEpBurstCntBackup[epNumber]   = UART_DmaEpBurstCnt[epNumber];
            ...
            /* Set destination address. */
            UART_CyDmaSetSrcAddress(channelNum, UART_DMA_DESCR0, (void*) &UART_ARB_EP_BASE.arbEp[epNumber].rwDr);
            UART_CyDmaSetSrcAddress(channelNum, UART_DMA_DESCR1, (void*) &UART_ARB_EP_BASE.arbEp[epNumber].rwDr);

            UART_CyDmaSetDstAddress(channelNum, UART_DMA_DESCR0, (void*) ((uint32) pData));
            UART_CyDmaSetDstAddress(channelNum, UART_DMA_DESCR1, (void*) ((uint32) pData + UART_DMA_BYTES_PER_BURST));

            /* Configure DMA descriptor. */
            UART_CyDmaSetConfiguration(channelNum, UART_DMA_DESCR0, UART_DMA_COMMON_CFG  | lengthDescr0 |
                                                    CYDMA_BYTE | CYDMA_WORD_ELEMENT | CYDMA_INC_DST_ADDR | CYDMA_INVALIDATE | CYDMA_CHAIN);

            /* Configure DMA descriptor. */
            UART_CyDmaSetConfiguration(channelNum, UART_DMA_DESCR1, UART_DMA_COMMON_CFG  | lengthDescr1 |
                                                    CYDMA_BYTE | CYDMA_WORD_ELEMENT | CYDMA_INC_DST_ADDR | CYDMA_INVALIDATE | CYDMA_CHAIN);
            ...
            if (UART_DmaEpBurstCntBackup[epNumber] > 1u)
            {
                UART_CyDmaValidateDescriptor(channelNum, UART_DMA_DESCR1);
            }
        ...
        #else
            (void) CyDmaChDisable(UART_DmaChan[epNumber]);
            (void) CyDmaTdSetConfiguration(UART_DmaTd[epNumber], length,  UART_DmaTd[epNumber], TD_TERMIN_EN | TD_INC_DST_ADR);
            (void) CyDmaTdSetAddress(UART_DmaTd[epNumber], LO16((uint32) &UART_ARB_EP_BASE.arbEp[epNumber].rwDr), LO16((uint32) pData));
...
void UART_LoadInEP16(uint8 epNumber, const uint8 pData[], uint16 length)
{
    ...
    if ((epNumber > UART_EP0) && (epNumber < UART_MAX_EP))
    {
        ...
        /* Set count and data toggle. */
        UART_SIE_EP_BASE.sieEp[epNumber].epCnt0 = (uint32) HI8(length) | UART_EP[epNumber].epToggle;
        UART_SIE_EP_BASE.sieEp[epNumber].epCnt1 = (uint32) LO8(length);
        ...
        if (NULL != pData)
        {
            ...
            {
                UART_ARB_EP16_BASE.arbEp[epNumber].rwDr16 = dataBuf[i];
            }
        }
        ...
        UART_SIE_EP_BASE.sieEp[epNumber].epCr0 = UART_EP[epNumber].epMode;
            ...
            if ((pData != NULL) && (length > 0u))
            {
                ...
                /* Configure source and destination. */
                UART_CyDmaSetSrcAddress(channelNum, UART_DMA_DESCR0, (void*) pData);
                UART_CyDmaSetDstAddress(channelNum, UART_DMA_DESCR0, (void*) &UART_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
                ...
                /* Generate DMA request. */
                UART_ARB_EP_BASE.arbEp[epNumber].epCfg |=  (uint32)  UART_ARB_EPX_CFG_DMA_REQ;
                UART_ARB_EP_BASE.arbEp[epNumber].epCfg &=  (uint32) ~UART_ARB_EPX_CFG_DMA_REQ;
            ...
            if (pData != NULL)
            {
                ...
                /* Set destination address. */
                UART_CyDmaSetDstAddress(channelNum, UART_DMA_DESCR0, (void*) &UART_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
                UART_CyDmaSetDstAddress(channelNum, UART_DMA_DESCR1, (void*) &UART_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
            ...
            else
            {
                ...
                if (length > 0u)
                {
                    ...
                    /* Adjust number of data elements transferred in last burst. */
                    UART_DmaEpLastBurstEl[epNumber] = (0u != UART_DmaEpLastBurstEl[epNumber]) ?
                                                                          ((UART_DmaEpLastBurstEl[epNumber] >> 1u) - 1u) :
                                                                           (UART_DMA_HALFWORDS_PER_BURST - 1u);

                    /* Get number of data elements to transfer for descriptor 0 and 1. */
                    lengthDescr0 = (1u == UART_DmaEpBurstCnt[epNumber]) ? UART_DmaEpLastBurstEl[epNumber] : (UART_DMA_HALFWORDS_PER_BURST - 1u);
                    lengthDescr1 = (2u == UART_DmaEpBurstCnt[epNumber]) ? UART_DmaEpLastBurstEl[epNumber] : (UART_DMA_HALFWORDS_PER_BURST - 1u);
...
/*******************************************************************************
* Function Name: UART_ReadOutEP16
****************************************************************************//**
*
*   This function performs different functionality depending on the Component’s
*   configured Endpoint Buffer Management. This parameter is defined in the
...
uint16 UART_ReadOutEP16(uint8 epNumber, uint8 pData[], uint16 length)
{
    ...
    adjLength =  length + ((uint32)length & 1u);
    ...
    #if (UART_EP_MANAGEMENT_MANUAL)
        {
            ...
            {
                dataBuf[i] = (uint16) UART_ARB_EP16_BASE.arbEp[epNumber].rwDr16;
            }
        }
        ...
        #if (UART_EP_MANAGEMENT_DMA_MANUAL)
        {
            ...
            /* Configure source and destination. */
            UART_CyDmaSetSrcAddress(channelNum, UART_DMA_DESCR0, (void*) &UART_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
            UART_CyDmaSetDstAddress(channelNum, UART_DMA_DESCR0, (void*) pData);

            /* Configure DMA descriptor. */
            UART_CyDmaSetConfiguration(channelNum, UART_DMA_DESCR0, UART_DMA_COMMON_CFG | (uint16)((adjLength >> 1u) - 1u) |
        ...
        #if (UART_EP_MANAGEMENT_DMA_AUTO)
        {
            ...
            /* Get number of full bursts. */
            UART_DmaEpBurstCnt[epNumber] = (uint8) (adjLength / UART_DMA_BYTES_PER_BURST);

            /* Get number of elements in last burst. */
            UART_DmaEpLastBurstEl[epNumber] = (uint8) (adjLength % UART_DMA_BYTES_PER_BURST);
            ...
            UART_DmaEpLastBurstEl[epNumber] |= UART_DMA_DESCR_16BITS;
            ...
            /* Set destination address. */
            UART_CyDmaSetSrcAddress(channelNum, UART_DMA_DESCR0, (void*) &UART_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
            UART_CyDmaSetSrcAddress(channelNum, UART_DMA_DESCR1, (void*) &UART_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
...
* \globalvars
*
*  \ref UART_EP[epNumber].apiEpState - set to NO_EVENT_PENDING
*
* \reentrant
*  No.
*
...
*    State                      |Description
*    ---------------------------|----------------------------------------------
*    UART_FORCE_J   | Force a J State onto the D+/D– lines
*    UART_FORCE_K   | Force a K State onto the D+/D– lines
*    UART_FORCE_SE0 | Force a Single Ended 0 onto the D+/D– lines
*    UART_FORCE_NONE| Return bus to SIE control
*
*
*******************************************************************************/
...
uint8 UART_GetEPAckState(uint8 epNumber) 
{
    ...
    if ((epNumber > UART_EP0) && (epNumber < UART_MAX_EP))
    {
        cr = UART_SIE_EP_BASE.sieEp[epNumber].epCr0 & UART_MODE_ACKD;
    }
...
*  --------------------------------------------|---------------------------
*  UART_DEVICE_STATUS_BUS_POWERED  | Set the device to bus powered
*  UART_DEVICE_STATUS_SELF_POWERED | Set the device to self powered
*
* \globalvars
*
*  \ref UART_deviceStatus - set power status
*
* \reentrant
*  No.
*
...
void UART_SetPowerStatus(uint8 powerStatus) 
{
    ...
    ***************************************************************************/
    ...
    {
        return ((0u != (UART_VBUS_STATUS_REG & UART_VBUS_VALID)) ? (uint8) 1u : (uint8) 0u);
    }
...
uint8 UART_RWUEnabled(void) 
{
    ...
    if (0u != (UART_deviceStatus & UART_DEVICE_STATUS_REMOTE_WAKEUP))
    {
        result = UART_TRUE;
    }
...
/*******************************************************************************
* Function Name: UART_GetDeviceAddress
****************************************************************************//**
*
...
{
    return (uint8)(UART_CR0_REG & UART_CR0_DEVICE_ADDRESS_MASK);
}


/*******************************************************************************
* Function Name: UART_EnableSofInt
****************************************************************************//**
*
...
void UART_EnableSofInt(void) 
{
    ...
    UART_INTR_SIE_MASK_REG |= (uint32) UART_INTR_SIE_SOF_INTR;
#else
...
/*******************************************************************************
* Function Name: UART_DisableSofInt
****************************************************************************//**
*
...
void UART_DisableSofInt(void) 
{
    ...
    /***************************************************************************
    * Function Name: UART_DetectPortType
    ************************************************************************//**
    *
    ...
    *   Return Value                      |Description
    *   ----------------------------------|-------------------------------------
    *   UART_BCD_PORT_SDP     | Standard downstream port detected
    *   UART_BCD_PORT_CDP     | Charging downstream port detected
    *   UART_BCD_PORT_DCP     | Dedicated charging port detected
    *   UART_BCD_PORT_UNKNOWN | Unable to detect charging port type (proprietary charger type)
    *   UART_BCD_PORT_ERR     | Error condition in detection process
    *
    *
    * \sideeffects
    *
    ...
    ***************************************************************************/
    uint8 UART_Bcd_DetectPortType(void)
    {
        ...
        uint8 result = UART_BCD_PORT_UNKNOWN;
        ...
        */
        bkPwrCtrl = UART_POWER_CTRL_REG;
        UART_POWER_CTRL_REG = UART_DEFAULT_POWER_CTRL_VBUS\
                            & (~UART_POWER_CTRL_ENABLE_VBUS_PULLDOWN)\
                            & (~UART_POWER_CTRL_ENABLE_DM_PULLDOWN);


        /* Enable PHY detector and single-ended and differential receivers.
         * Enable charger detection.  */
        UART_POWER_CTRL_REG |= UART_DEFAULT_POWER_CTRL_PHY\
                                         | UART_POWER_CTRL_ENABLE_CHGDET;
        ...
        */
        #if defined (UART_NO_DCD)
        CyDelay(UART_BCD_TIMEOUT);
        #else
        /* DCD implementation:*/

        {
            uint16 timeout = UART_BCD_TIMEOUT;
            uint8 connectionApproved = 0u;
            uint8 connected = 0u;

            /*   BCD spec 1.2: Turns on Idp_src and D- pull-down resistor */
            UART_POWER_CTRL_REG |= UART_POWER_CTRL_ENABLE_DM_PULLDOWN;
            UART_CHGDET_CTRL_REG |= UART_CHGDET_CTRL_DCD_SRC_EN;

            /* BCD spec 1.2: Waits for D+ to be low for a time of Tdcd_dbnc*/
            while ((0u != timeout) && (0u == connectionApproved))
            {
                if (0u == (UART_USBIO_CR1_REG & UART_USBIO_CR1_DP0))
                {
                    connected++;
                }
                else
                ...
                connectionApproved = (UART_BCD_TDCD_DBNC < connected) ? 1u:0u;
                CyDelay(1u);
                timeout--;
            }
        ...
        UART_CHGDET_CTRL_REG = UART_CHGDET_CTRL_PRIMARY;
        CyDelay(UART_BCD_PRIMARY_WAIT);
        cr1RegVal = UART_USBIO_CR1_REG;

        /* Check is it SDP or DCP/CDP, read comparator 2 output. */
        if (0u == (UART_CHGDET_CTRL_REG & UART_CHGDET_CTRL_COMP_OUT))
        {
            /* Check status of D- line. */
            if (0u == (cr1RegVal & UART_USBIO_CR1_DM0))
            {
                result = UART_BCD_PORT_SDP;
            }
            else
            {
        ...
        {
            UART_CHGDET_CTRL_REG = UART_CHGDET_CTRL_DEFAULT \
                                                | UART_CHGDET_CTRL_SECONDARY;
            CyDelay(UART_BCD_SECONDARY_WAIT);
            cr1RegVal = UART_USBIO_CR1_REG;
            ...
            {
                /* Check status of D+ line. */
                if (0u == (cr1RegVal & UART_USBIO_CR1_DP0))
                {
                    result = UART_BCD_PORT_CDP;
                }
                else
                {
                    /* ERROR: such combination is impossible. Abort charger
            ...
            else
            {
                ...
                else
                {
                    ...
                    */
                    result = UART_BCD_PORT_UNKNOWN;
                }
            }
        }

        /* Restore CHGDET_CTRL. */
        UART_CHGDET_CTRL_REG = 0u;

        /*Revert registers back*/
        UART_POWER_CTRL_REG = bkPwrCtrl;
        UART_USBIO_CR1_REG |= (uint32) UART_USBIO_CR1_IOMODE;
        UART_USB_CLK_EN_REG = ~UART_USB_CLK_CSR_CLK_EN;
...
#if (UART_LPM_ACTIVE)
    /***************************************************************************
    * Function Name: UART_Lpm_GetBeslValue
    ************************************************************************//**
    *
    ...
    ***************************************************************************/
    ...
    {
        return (uint32) (UART_LPM_STAT_REG & UART_LPM_STAT_LPM_BESL_MASK);
    }


    /***************************************************************************
    * Function Name: UART_Lpm_RemoteWakeUpAllowed
    ************************************************************************//**
    *
    ...
    ***************************************************************************/
    ...
    {
        return (uint32) (UART_LPM_STAT_REG & UART_LPM_STAT_LPM_REMOTE_WAKE);
    }


    /***************************************************************************
    * Function Name: UART_Lpm_SetResponse
    ************************************************************************//**
    *
    ...
    *       - UART_LPM_REQ_ACK - next LPM request will be
    *                                           responded with ACK
    *       - UART_LPM_REQ_NACK - next LPM request will be
    *                                           responded with NACK
    *       - UART_LPM_REQ_NYET - next LPM request will be
    *                                           responded with NYET
    *
    ***************************************************************************/
    void UART_Lpm_SetResponse(uint32 response)
    {
        uint32 lpmCtrl = UART_LPM_CTRL_REG & (uint32) ~UART_LPM_CTRL_ACK_NYET_MASK;

        UART_LPM_CTRL_REG = lpmCtrl | ((uint32) response & UART_LPM_CTRL_ACK_NYET_MASK);
    }


    /***************************************************************************
    * Function Name: UART_Lpm_GetResponse
    ************************************************************************//**
    *
    ...
    *       - UART_LPM_REQ_ACK - next LPM request will be responded
    *                                        with ACK
    *       - UART_LPM_REQ_NACK - next LPM request will be responded
    *                                        with NACK
    *       - UART_LPM_REQ_NYET - next LPM request will be responded
    *                                        with NYET
    *
    ***************************************************************************/
    uint32 UART_Lpm_GetResponse(void)
    {
        ...
        return  ((uint32) UART_LPM_CTRL_REG & (uint32)UART_LPM_CTRL_ACK_NYET_MASK);
    }
